...
class RollingForecast(ForecastingStrategy):
    ...
    def _eval_sample(
        ...
        if hasattr(model, "prepare_query_context_array"):
            forecast_query_contexts = []
            for index in itertools.islice(index_list, num_rollings):
                train, _ = split_time(series, index)
                target_train, exog_train = split_channel(train, target_channel)
                prepared = model.prepare_query_context_array(
                    target_train.values,
                    None if exog_train is None else exog_train.values,
                )
                contexts = prepared[-model.config.seq_len :]
                if contexts.shape[0] == model.config.seq_len:
                    forecast_query_contexts.append(contexts[None, ...])
            self._precompute_forecast_shortlists(
                model,
                np.concatenate(forecast_query_contexts, axis=0)
                if forecast_query_contexts
                else None,
            )
    ...
    def _eval_batch(
        ...
        if hasattr(model, "prepare_query_context_array"):
            window_batch = batch_maker.make_batch_predict(
                len(index_list), model.config.seq_len
            )
